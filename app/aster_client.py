...
class AsterClient:
    def __init__(self, base_url: str = "https://fapi.asterdex.com", timeout_seconds: int = 12) -> None:
    ...
    def get_overview(self, symbol: str = "ETHUSDT") -> Dict[str, Any]:
        ticker = self._get("/fapi/v3/ticker/24hr", {"symbol": symbol})
        premium = self._get("/fapi/v3/premiumIndex", {"symbol": symbol})
        open_interest = self._get("/fapi/v3/openInterest", {"symbol": symbol})
        return {
            "symbol": symbol,
    ...
    def get_klines(self, symbol: str = "ETHUSDT", interval: str = "5m", limit: int = 400) -> List[List[Any]]:
        ...
        data = self._get("/fapi/v3/klines", {"symbol": symbol, "interval": interval, "limit": safe_limit})
    ...
    def get_depth(self, symbol: str = "ETHUSDT", limit: int = 20) -> Dict[str, Any]:
        ...
        for val in tries:
            try:
                data = self._get("/fapi/v3/depth", {"symbol": symbol, "limit": val})
    ...
    def get_exchange_info(self) -> Dict[str, Any]:
        data = self._get("/fapi/v3/exchangeInfo", {})
        return data if isinstance(data, dict) else {}
    ...
    def get_usdt_symbols_ranked(self, pinned_symbols: List[str] | None = None) -> List[str]:
        ...
        ticker_data = self._get("/fapi/v3/ticker/24hr", {})
